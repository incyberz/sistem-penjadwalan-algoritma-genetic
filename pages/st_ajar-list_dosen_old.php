...
if (mysqli_num_rows($q)) {
  ...
  while ($d = mysqli_fetch_assoc($q)) {
    ...
    $sum_sks = $d['sum_sks'] ? "<span class='blue bold'><span id=count_mk__$d[id]>$d[sum_sks]</span> SKS</span>" : '-';

    $list_dosen .= "
      <tr>
        <td>$i</td>
        <td>
          <a href='?st_ajar&id_dosen=$d[id]' class='pointer a_dosen' id=a_dosen__$d[id]>
