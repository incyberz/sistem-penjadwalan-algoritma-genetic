...
if ($kelas['id_dosen_wali']) {
  $status_syarat = "<b>Dosen Wali:</b> $kelas[nama_dosen_wali] <span class=btn_aksi id=form_ubah_id_dosen_wali__toggle>$img_edit</span>";
  $hideit = 'hideit';
} else {
  $hideit = '';
}
...
while ($d = mysqli_fetch_assoc($q)) {
  $opt_dosen .= !$checked ? '' : "<option value=$d[id]>$d[nama] - NIDN. $d[nidn]</option>";
}
